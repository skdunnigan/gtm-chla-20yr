...
pi <- fulc_tbl(dat = mdat.pi) %>% mutate(site = "PI", dev.avg = fulcrum - mean(.$fulcrum, na.rm = T)) 
ss <- fulc_tbl(dat = mdat.ss) %>% mutate(site = "SS", dev.avg = fulcrum - mean(.$fulcrum, na.rm = T))
fm <- fulc_tbl(dat = mdat.fm) %>% mutate(site = "FM", dev.avg = fulcrum - mean(.$fulcrum, na.rm = T))
pc <- fulc_tbl(dat = mdat.pc) %>% mutate(site = "PC", dev.avg = fulcrum - mean(.$fulcrum, na.rm = T))
...
ggsave(f_plot, filename = here('output', 'figures', 'finals', 'figure7.png'),
       dpi = 600, units = "in",
       width = 6.5, height = 5)

# linear model on all stations
# a <- lm(fulcrum ~ year, data = fulcrums)
# broom::glance(a)
