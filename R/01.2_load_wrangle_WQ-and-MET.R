...
pi <- SWMPr::import_local(path = here::here('data'), station_code = 'gtmpiwq') %>% 
      SWMPr::qaqc(qaqc_keep = c('0', '2', '3', '4', '5')) 

ss <- SWMPr::import_local(path = here::here('data'), station_code = 'gtmsswq') %>% 
      SWMPr::qaqc(qaqc_keep = c('0', '2', '3', '4', '5'))

fm <- SWMPr::import_local(path = here::here('data'), station_code = 'gtmfmwq') %>% 
      SWMPr::qaqc(qaqc_keep = c('0', '2', '3', '4', '5')) %>% 
      dplyr::select(1, sal, do_mgl, turb)

pc <- SWMPr::import_local(path = here::here('data'), station_code = 'gtmpcwq') %>% 
      SWMPr::qaqc(qaqc_keep = c('0', '2', '3', '4', '5')) %>% 
      dplyr::select(1, sal, do_mgl, turb)
